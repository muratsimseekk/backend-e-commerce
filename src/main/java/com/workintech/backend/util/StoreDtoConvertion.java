...
public class StoreDtoConvertion {
    ...
    public static List<StoreResponse> converStoreList (List<Store> stores){
    ...
    public static StoreResponse converStore(Store store){
        return new StoreResponse(store.getId(), store.getName(), store.getPhone()
                , store.getTaxNo(), store.getBankAccount(), store.getProducts(),store.getUser().getId());
    }
